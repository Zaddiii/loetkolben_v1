...
static void MX_TIM15_Init(void)
{
  ...
  if (HAL_TIM_PWM_ConfigChannel(&htim15, &channel_config, TIM_CHANNEL_1) != HAL_OK)
  {
    Error_Handler();
  }
...
static void MX_TIM17_Init(void)
{
  TIM_IC_InitTypeDef channel_config = {0};

  /* Prescaler=63 → 64 MHz / 64 = 1 MHz → 1 µs resolution.
     Period 0xFFFF allows up to 65 ms per edge interval before wrap-around.
     Period-based RPM is computed in HAL_TIM_IC_CaptureCallback. */
  ...
  channel_config.ICPolarity = TIM_INPUTCHANNELPOLARITY_RISING;
  channel_config.ICSelection = TIM_ICSELECTION_DIRECTTI;
  channel_config.ICPrescaler = TIM_ICPSC_DIV1;
  channel_config.ICFilter = 12;

  if (HAL_TIM_IC_ConfigChannel(&htim17, &channel_config, TIM_CHANNEL_1) != HAL_OK)
  {
    Error_Handler();
  }
}
